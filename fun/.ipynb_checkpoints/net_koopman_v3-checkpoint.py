...
def k_matrix(shape, name='k_matrix'):
    """
    初始化一个shape=shape的方阵
    v3 改成返回值是 shape 2 的形式 使得整个工程 支持 复数特征值
    """
    k_init = tf.random_normal_initializer(stddev=0.05)
    return tf.Variable(initial_value=k_init(shape=(shape, 2), dtype='float32'), trainable=True, name=name)


def koopman_next(data, omegas, dt=0.4):
    """
    计算在koopman空间下一时刻下的状态
    v3 利用 for 循环 支持 对于整个高维空间的时间演化
    这里借鉴了 NC 开源代码中的一些写法
    （Deep learning for universal linear embeddings of nonlinear dynamics）
    :param data: 高维空间数据 shape = None sp z-dim 还有可能是二维 None z-dim   一般来说是300 最好是偶数 因为这里没有处理升维是奇数的情况
    :param omegas: K矩阵的特征值 这里为了兼容v1版本 就不改了 shape = z-dim/2 2 , 两个维度分别做为复数的实部和虚部
    :param dt: 采样时间 对于目前来说，因为数据的采样时间是0.4

    """
    # 保证两个维度相等
    if data.shape[-1] != omegas.shape[0]:
        assert 1 == 0
    else:
        z_dim = data.shape[-1]
    # omegas 第一列是mu 实部 第二列是omega 虚部
    scale = tf.exp(omegas[:, 0] * dt)  # shape 300,
    entry_cos = tf.multiply(scale, tf.cos(omegas[:, 1] * dt))  # shape 150,
    entry_sin = tf.multiply(scale, tf.sin(omegas[:, 1] * dt))
    row1 = tf.stack([entry_cos, -entry_sin], axis=1)  # shape 150, 2
    row2 = tf.stack([entry_sin, entry_cos], axis=1)

    tt = tf.concat([row1, row2], axis=0)  # shape 300 2   不能这样 这样就都先是 row1 然后才是 row2 ,
    # 应该是两个换着来

    return tf.reduce_sum(tf.matmul(data, tt), axis=-1)  # 这里把最后一个2 的维度消去 这样最后的shape 和data的一样


def koopman_next_v4(data, omegas, dt=0.4):
    """
    计算在koopman空间下一时刻下的状态
    v4 利用 for 循环 支持 对于整个高维空间的时间演化
    这个版本的问题可能是 for 循环太耗时间了 有机会再改吧。
    这里借鉴了 NC 开源代码中的一些写法
    （Deep learning for universal linear embeddings of nonlinear dynamics）
    :param data: 高维空间数据 shape = None sp z-dim 还有可能是二维 None z-dim   一般来说是300 最好是偶数 因为这里没有处理升维是奇数的情况
    :param omegas: K矩阵的特征值 这里为了兼容v1版本 就不改了 shape = z-dim/2 2 , 两个维度分别做为复数的实部和虚部
    ...
    """
    # 保证两个维度相等
    if data.shape[-1] != omegas.shape[0] * 2:
        assert 1 == 0
    else:
        z_dim = data.shape[-1]
    #
    # 还要两个版本 一个是 data 是三维 一个是 data 是2维   这里用 ... 处理 三维和二维的不同
    z_next_list = []
    for i in range(int(z_dim/2)):
        scale = tf.exp(omegas[i, 0] * dt)  # shape 1,
        entry_cos = tf.multiply(scale, tf.cos(omegas[i, 1] * dt))  # shape 1,
        entry_sin = tf.multiply(scale, tf.sin(omegas[i, 1] * dt))
        z_part_next_1 = data[..., i*2] * entry_cos + data[..., i*2 + 1] * entry_sin
        z_next_list.append(z_part_next_1)
        z_part_next_2 = -data[..., i*2] * entry_sin + data[..., i*2 + 1] * entry_cos
        z_next_list.append(z_part_next_2)

    return tf.stack(z_next_list, axis=-1)  # 这里把list的维度放在最后 这样最后的shape 和data的一样
...
class Koopman(ks.Model, abc.ABC):
    ...
    def call(self, inputs):
        return koopman_next_v4(inputs, self.k_matrix)
