...
# Author     ：pang
# version    ： 2.0
# Description：v1: 实现非受力系统的基于深度学习的koopman算符的基础工作。比如定义编码器 解码器 koopman空间 以及相关损失函数等。
               v2: 尝试解决模型用一个model, 修复之前用model没有可训练变量的bug(还是不行 那就这样吧 懒得管了)
"""
"""
时间 23年3月5日
内容 定义网络结构文件
...
class MLPLayers(ks.layers.Layer):
    ...
    def build(self, input_shape=1):
        ...
        for i in range(self.net_len - 1):
            self.w_dict[self.name_ + '_w_%d' % (i + 1)] = tf.Variable(
                initial_value=self.w_init(shape=(self.mlp_list[i], self.mlp_list[i + 1]),
                                          dtype='float32', ),
                trainable=True,
                name=self.name_ + '_w_%d' % (i + 1))
            self.b_dict[self.name_ + '_b_%d' % (i + 1)] = tf.Variable(
                initial_value=self.b_init(shape=[self.mlp_list[i + 1], ],
                                          dtype='float32'),
                trainable=True,
                name=self.name_ + '_b_%d' % (i + 1))
...
class Koopman(ks.Model, abc.ABC):
    ...
    def __init__(self, encoder_list, decoder_list, act_f, name='koopman', ):
        super(Koopman, self).__init__()

        self.shape = encoder_list[-1]
        self.encoder_list = encoder_list
        self.decoder_list = decoder_list
        self.active_f = act_f   # 这里不用做选择 把选择放在 mlp层里再把字符串转换为激活函数

        self.k_matrix = k_matrix(self.shape, name='koopman_matrix')
    ...
    def call(self, inputs, sp):
        pre_dict = {}

        z = self.encoder(inputs)
        x_recon = self.decoder(z)

        z_one_shift = koopman_next(z, self.k_matrix)
        x_recon_one_shift = self.decoder(z_one_shift)

        z_mult_shift_list = []
        z_start = z[:, 0, :]
        for j in range(sp):
            z_start = koopman_next(z_start, self.k_matrix)
...
if __name__ == '__main__':
    t = MLPLayers(act_fun='relu', mlp_list=[30, 30, 30])
    t.name_
    t.act_fun
    t.mlp_list
    pass
