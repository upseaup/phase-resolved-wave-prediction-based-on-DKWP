...
时间 23年10月8日
1.更新koopma实现 因为现在是区间对区间预报 所以可以用完整的Koopman， 需要把之前那些loss都考虑到
2.主要借鉴之前的程序 F:\旧工程存档\code_project\koopman_keras\verson\ver-1-2（car）\forward_car.py
"""
#
def mish(x):
...
class Encoder_decoder(ks.Model, abc.ABC):
    ...
    需要注意的是网络输出是什么，这影响这损失函数应该如何写
    """
    def __init__(self, encoder_list, decoder_list, active_f='relu', w_init=tf.keras.initializers.GlorotUniform, sp=1):
        super(Koopman, self).__init__()
        ...
        self.encoder = MLPLayers(self.encoder_list, act_fun=self.active_f, w_init=w_init, name='encoder')
        self.k_mat = k_matrix(self.encoder_list[-1])
        self.decoder = MLPLayers(self.decoder_list, act_fun=self.active_f,  w_init=w_init, name='decoder')
...
def diag_matrix(shape=None):
    ...
    # 这个有个问题 就是说 虽然得到的是对角阵 但是训练的时候会把其他零元素也训练了
...
def diag_row(shape=None):
    ...
    for i in range(shape):
        ...
        diag_list.append(tf.Variable(mu,  trainable=True, dtype='float32'))
    return diag_list 
...
def diag_matrix_mask(shape=None):
    ...
    # diag = tf.keras.initializers.Constant(diag,  trainable=False, dtype='float32')
...
def koopman_next_v2(data, mat):
    ...
    """
    diag_list = mat[0]
    diag_matrix_mask = mat[1]
    diag_matrix_len = diag_matrix_mask.shape[0]
    # data shape n, sp, z-dim
    z_next_list = []
    for i in range(diag_matrix_len):
        # 提取对应维度 并降维
        z_next_part = tf.reduce_max(tf.matmul(data, diag_matrix_mask[:, i:i+1]), axis=2)
        # 乘特征值 并升维度
        z_next_part = tf.expand_dims(z_next_part, -1)
        z_next_part = tf.matmul(z_next_part, diag_list[i])
        z_next_list.append(z_next_part)
    
    return tf.concat(z_next_list, axis=2) 
...
#            两者在高维空间演化没有误差时 是等价的，
"""
class Koopman_v2(ks.Model, abc.ABC):
    """
    v2版本：完整的koopman 三种损失函数都有
    时间：  2023年10月8日
    sp : 内部循环的次数 
    需要注意的是网络输出是什么，这影响这损失函数应该如何写
    """
    def __init__(self, encoder_list, decoder_list, active_f='relu', sp=1):
        super(Koopman_v2, self).__init__()
        ...
        self.encoder = MLPLayers(self.encoder_list, act_fun=self.active_f, name='encoder')
        self.diag = diag_matrix(self.encoder_list[-1])
        self.decoder = MLPLayers(self.decoder_list, act_fun=self.active_f, name='decoder')

    def call(self, inputs, training=None, mask=None):
        # 考虑下 x 的shape 应该是 n sp a-b
        # 对应   z 的shape 应该是 n sp z-dim
        # 其实原来的论文应该都没有考虑过 one_shift这种东西 
        x = inputs
        pre_dict = {}
        ...
        z_one_shift = koopman_next(z, mat_a) 
        x_recon_one_shift = self.decoder(z_one_shift)

        z_mult_shift_list = []
        z_start = z[:, 0, :]
        for i in range(self.sp):
            z_start = tf.matmul(z_start, mat_a)
            z_mult_shift_list.append(z_start)
        z_mult_shift = tf.stack(z_mult_shift_list, axis=1)
        x_recon_mult_shift = self.decoder(z_mult_shift)
...
def loss_fun_model(x, output_dict, sp):
    """
